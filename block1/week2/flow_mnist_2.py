...
class MaskedCouplingLayer(nn.Module):
    ...
    def inverse(self, x):
        ...
        log_det_J = -((1-m) * self.scale_net(m*z)).sum(dim=-1)
        return z, log_det_J
...
if __name__ == "__main__":
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, persistent_workers=True)
    ...
    if args.mode == 'train':
        ...
        model.eval()
        with torch.no_grad():
            samples = (model.sample((2*2,))).cpu()


        save_image(samples.view(-1, 1, 28, 28), args.samples, nrow=2)
