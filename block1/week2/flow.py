...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        ...
        y = self.mask * z
        sy = self.scale_net(y)
        x = y + (1 - self.mask)*(z * torch.exp(sy) + self.translation_net(y))
        log_det_J = torch.inner((1-self.mask), sy)
        return x, log_det_J
    ...
    def inverse(self, x):
        ...
        y = self.mask * x
        sy = self.scale_net(y)
        z = y + (1 - self.mask)*((torch.exp(-sy)) * (x - self.translation_net(y)))
        log_det_J = 1 / torch.inner((1-self.mask), sy)
        return z, log_det_J
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='sample', choices=['train', 'sample'], help='what to do when running the script (default: %(default)s)')
    ...
    parser.add_argument('--model', type=str, default='model_10_16_4.pt', help='file to save model to or load model from (default: %(default)s)')
    parser.add_argument('--samples', type=str, default='samples_10_16_4.png', help='file to save samples in (default: %(default)s)')
