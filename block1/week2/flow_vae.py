...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        ...
        x = z_m + (1-m) * (z * s.exp() + self.translation_net(z_m))
        log_det_J = ((1-m) * s).sum(dim=-1)
        return x, log_det_J
    
    def inverse(self, x, return_scale=False):
        """
        ...
        z = x_m + (1-m) * ((x - self.translation_net(x_m)) * (-s).exp())
        # NOTE: No need to evaluate self.scale_net(m*z), 
        #   because it is equal to s via the mask and identity transform.
        log_det_J_inv = -((1-m) * s).sum(dim=-1)
        
        if return_scale:
            return z, log_det_J_inv, m*torch.ones_like(z) + (1-m)*(-s).exp()
        else:
...
class Flow(nn.Module):
    ...
    def inverse(self, x, return_scale=False):
        ...
        for T in reversed(self.transformations):
            output = T.inverse(x, return_scale)

            if return_scale:
                z, log_det_J_inv, s = output
                scale *= s
            else:
                z, log_det_J_inv = output

            x = z
            sum_log_det_J_inv += log_det_J_inv
        
        if return_scale:
            return z, sum_log_det_J_inv, scale
        else:
            return z, sum_log_det_J_inv
